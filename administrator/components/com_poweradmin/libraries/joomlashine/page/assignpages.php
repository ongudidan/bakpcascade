...
class JSNAssignpages extends JSNDatabase{
	...
	public function menuTypeDropDownList()
	{
		...
		for( $i = 0; $i < count($menus); $i++){
			$titleLists[$i] = JSNHtmlHelper::openTag('div', array('class'=>'jsn-menutype-title', 'id'=>'jsn-menutype-title-'.$menus[$i]->id, 'menutype'=> $menus[$i]->menutype, 'menuid'=>$menus[$i]->id, 'menutitle'=>$menus[$i]->title))
			            		.$menus[$i]->title
			            		.JSNHtmlHelper::closeTag('div');
		    $dropDownList[] = JSNHtmlHelper::openTag('li', array('id'=>'dropdownmenutype-'.$menus[$i]->id))
		                 .JSNHtmlHelper::openTag('a', array('class'=>'text'))
		                    .$menus[$i]->title
		                 .JSNHtmlHelper::closeTag('a')
		              .JSNHtmlHelper::closeTag('li');
		}
