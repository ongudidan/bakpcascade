		...
		if ( !$this->_dispatch ){
			if(class_exists('JSNHtmlAsset')){
				if(method_exists('JSNHtmlAsset','addScript') && method_exists('JSNHtmlAsset','addStyle')){
		            ...
		            $this->addScript(JURI::root(true) . '/media/jui/js/jquery.js');
		            $this->addScript( JSN_POWERADMIN_LIB_JSNJS_URI . 'jsn.jquery.noconflict.js');
		            JSNHtmlAsset::addScript(JSN_FRAMEWORK_ASSETS . '/3rd-party/jquery-ui/js/jquery-ui-1.9.0.custom.min.js');
		            $this->addScript(JURI::root(true) . '/media/jui/js/jquery.ui.sortable.js');
		            ...
		            $this->addStyleSheet(JSN_POWERADMIN_STYLE_URI ."poweradmin.css");
				}
			}
