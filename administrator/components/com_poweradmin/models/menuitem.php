...
class PoweradminModelMenuitem extends JModelList
{
	...
	public static function loadMenuCustomAssets($menuId, $type = 'css')
	{
		...
		return count($assets)? json_decode($assets->assets) : array();
	}
	 ...
	 */
	public static function saveMenuAssets($menuId, $assets = "", $type = "css")
	{		
		$dbo = JFactory::getDbo();	
		
		if($assets){
			if(self::loadMenuCustomAssets($menuId, $type)){
				$query 	= "UPDATE #__jsn_poweradmin_menu_assets SET assets='" . mysql_real_escape_string($assets) . "' WHERE type = '" . $type . "' AND menuId=" . (int)$menuId;
			}else{
				$query 	= "INSERT INTO #__jsn_poweradmin_menu_assets(menuId, assets, type) VALUES ('" . (int)$menuId . "', '" . mysql_real_escape_string($assets) . "', '" . $type . "') ";
			}	
		}else{
