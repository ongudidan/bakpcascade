	...
	*/
	$.fn.jsnSubmenu = function(ops){
		...
		*/		
		...
		{
			var item = $('<li/>').appendTo(elements).text(text);
			for(key in attrs){
		...
		*/
		this.addParentItem = function(text, attrs){
			...
			var moreElements = $('<ul />', 
			{
				'class': 'jsn-submenu-subpanel ' + self.attr('class')
			}).appendTo(parentItem)
			.css({
				'position'  : 'absolute',
				'left'      : self.width() - 5,
			...
			*/			
			parentItem.mouseenter(function(e){				
				...
				if (self.offset().left + self.width() + moreElements.width() > $(window).width()){
					moreElements.css('left', - ( moreElements.width() + 2 ));
				}else{
					moreElements.css('left', self.width() - 6);
				}

				if ( self.offset().top + parentItem.position().top + moreElements.height() > $(window).height() ){
					var marginTop = moreElements.height() / 2 + 10;
					moreElements.css('top', '-' + marginTop + 'px');
				}
			});
			/**
			...
			*/
			parentItem.addItem = function(text, attrs){
				var item = $('<li/>').appendTo(moreElements).text(text);
				for(key in attrs){
					item.attr(key, attrs[key]);
				}
			...
			*/
			parentItem.addDivider = function(){
				return  $('<li/>').appendTo(moreElements)
								  .addClass('jsn-submenu-divider')
								   html('&nbsp;');
			}; 
		...
		*/
		this.removeDividers = function(){
			$('.jsn-submenu-divider', elements).remove();
		};
		/**
		* 
		* Hide divider
		...
		*/
		this.hideDividers = function(){
			$('.jsn-submenu-divider', elements).css('display', 'none');
		};
		/**
		* 
		*
		...
		*/
		this.showDividers = function(){
			$('.jsn-submenu-divider', elements).css('display', 'block');
		};
		/**
		* 
		* Get an menu item
		...
		* @return: jQuery Object
		*/
		this.getItem = function(text, index)
		{
			var found = [$({})],
			    i = 0;
			index = ( index == undefined ) ? 0 : index;
			$('li', elements).each(function(){
				var tmp = $(this).clone(true);
				tmp.children().each(function(){
					$(this).remove();
				});
				if (tmp.text().trim().toLowerCase() == text.trim().toLowerCase()){
					found[i++] = $(this);
				}
				tmp.remove();
			});
			...
			if(!found[index]){
				return this.addSwapEvents( found[0] );
			}		
			
			return this.addSwapEvents( found[index] );
		};
		/**
		* 
		* Show item
		...
		*/
		this.showItem = function(text, index){
			return this.getItem(text, index).show();
		};
		/**
		* 
		* Hide item
		...
		*/
		this.hideItem = function(text, index){
			return this.getItem(text, index).hide();
		};
		/**
		* 
		* Remove item
		...
		*/
		...
		{				
			...
			self.show().css({'height': elements.height(), 'z-index': $.topZIndex()});
