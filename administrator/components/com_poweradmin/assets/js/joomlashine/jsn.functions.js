...
(function($){	
	...
	*/
	$.setPosition = function(moduleid, position){
		$.post
		(
			baseUrl+'administrator/index.php?option=com_poweradmin&view=changeposition&task=changeposition.setPosition',
			{
				moduleid: moduleid,
				position: position
			}
	...
	*/
	...
	$.fn.showLoading = function(ops){
		...
		if ( $(this).find('.ui-widget-overlay').length == 0 ){
			...
			if ( _ops.showImgLoading ){
				...
				$('<img />', {
					'src' : baseUrl+'administrator/components/com_poweradmin/assets/images/ajax-loader.gif'
				})
    ...
    $.fn.showImgStatus = function( options ){
    	...
    	if ( ops.status == 'request' ){
    		var img = $('<img />', {
    			'src': baseUrl+'administrator/components/com_poweradmin/assets/images/ajax-request.gif'
    		});
    	}else if( ops.status == 'success' ){
    		var img = $('<img />', {
    			'src': baseUrl+'administrator/components/com_poweradmin/assets/images/ajax-success.gif'
    		});
    	}else if( ops.status == 'error' ){
    		var img = $('<img />', {
    			'src': baseUrl+'administrator/components/com_poweradmin/assets/images/ajax-error.gif'
    		});
    	}else{
    		ajaxrequest.html('');
    	}
