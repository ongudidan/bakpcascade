	...
	var assignmentPages = {
		...
		*/
		initEvents: function(){
			...
			$('#assignment-dropdown-list').change(function(){
			...
			$(window).bind("jsn.jstree.loaded", function(){
				$('.jstree-icon',$('#jsn-assign-module a')).remove();
			...
			$('#jsn-toggle-publish-module').unbind('click').click(function(){
			...
			if ( $.jStorage.get('menuitem_publishing_options') ){
				$('#jsn-toggle-publish-module').addClass('active').attr('title', JSNLang.translate('TITLE_HIDE_UNPUBLISHED'));
			}else{
				$('#jsn-toggle-publish-module').removeClass('active').attr('title', JSNLang.translate('TITLE_SHOW_UNPUBLISHED'));
			}
			
			//Show/hide dropdown select menutypes
			$('.jsn-menu-selector-container').unbind("scroll").bind("scroll", function () {
				var menuTypeSelector = $('#jsn-assign-module').find('.jsn-menu-selector');
		...
		*/
		jstreeInstance: function(){
			...
			console.log($('.jsn-menuitem-assignment'));
			//Init jstree
