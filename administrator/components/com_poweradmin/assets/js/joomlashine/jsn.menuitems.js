		...
		menuActions:{
						...
						open  : function(){
				 		...
				 		buttons:{				 			
				 			'Save': function(){
								...
								pop.submitForm('item.apply', 'Save', function(){
							...
							'Close': function(){
								...
								$.post( baseUrl+'administrator/index.php?option=com_poweradmin&task=menuitem.checkinmenuitem',{itemid:itemid}, function(){
									$._menuitems.resetMenu(mid);
			...
			*/
			publishing: function (obj) {
				...
				$.post( 
					baseUrl+'administrator/index.php?option=com_poweradmin&task=menuitem.menuitempublishing', 
					{
						itemid : id[2], 
						publish: id[0]
					}
			...
			*/
			checkin: function (obj) {
				...
				$.post( 
					baseUrl +'administrator/index.php?option=com_poweradmin&task=menuitem.checkinmenuitem', 
					{
						itemid: id[2]
					}
				).success(function(res){
			...
			*/
			makehome: function(obj){
				...
				$.post( 
					baseUrl + 'administrator/index.php?option=com_poweradmin&task=menuitem.setdefault', 
					{
						itemid: id[2]
					}
				).success(function(res){
			...
			*/
			trash: function (obj) {
				...
				if (answer){
					...
					$.post( 
						baseUrl+'administrator/index.php?option=com_poweradmin&task=menuitem.trashmenuitem', 
						{
							itemid:id[2]
						}
					).success(function(res){
						...
						scrollContent: false,
				 		...
				 		buttons:{				 			
				 			'Save': function(){
								...
								_this.addClass('jsn-loading');
								pop.submitForm('menuitem.saveassets', 'Save', function(){
									_this.removeClass('jsn-loading');
									_this.dialog("close");
								});
							},
			...
			*/
			rebuild: function (obj) {
				...
				$.post(
					baseUrl+'administrator/index.php?option=com_poweradmin&task=menuitem.rebuilditem', 
					{
						itemid: id[2]
					}
				).success(function(res){
	...
	*/
	$.fn.jstreeInstance = function(){
		...
		$(this).jstree({
			...
			"plugins" : ["themes","crrm", "hotkeys", "contextmenu", "html_data",  "ui", "dnd"],
		...
		*/
		.bind("move_node.jstree", function (e, data) {
			...
			data.rslt.o.each(function (i) {
				...
				$.post
				(
					baseUrl+'administrator/index.php?option=com_poweradmin&task=menuitem.moveItem',
					{
						itemid   : move_id, 
						parentid : parent_id, 
						orders   : orders
		...
		*/
		...
		$(document).unbind("showmenu.jstree").bind("showmenu.jstree", function(event, obj, items){
			items['ccp']['submenu']['MakehomeDisabled'].show = false;
			items['ccp']['submenu']['TrashDisabled'].show = false;
			items['ccp']['submenu']['CheckinDisabled'].show =  false;
			items['UnpublishDisabled'].show = false;
			...
			if ($(obj).hasClass('unpublish')){
				items['Unpublish'].label = JSNLang.translate( 'TITLE_SUBMENU_PUBLISH_MENU_ITEM' );
				toggleSubmenuItem (items['ccp']['submenu']['Makehome'], items['ccp']['submenu']['MakehomeDisabled'])
			}else if( $(obj).hasClass('default') ){
				toggleSubmenuItem (items['Unpublish'], items['UnpublishDisabled'], true);
				toggleSubmenuItem (items['ccp']['submenu']['Makehome'], items['ccp']['submenu']['MakehomeDisabled']);
				toggleSubmenuItem (items['ccp']['submenu']['Trash'], items['ccp']['submenu']['TrashDisabled'], true);

				$(document).unbind("context_show.vakata").bind("context_show.vakata", function(){
					$._menuitems.jstreeContextMenuShowing = true;
				});
			}else if($(obj).hasClass('checked-in')){
				toggleSubmenuItem (items['ccp']['submenu']['Checkin'], items['ccp']['submenu']['CheckinDisabled'], true);
				$(document).unbind("context_show.vakata").bind("context_show.vakata", function(){
					$._menuitems.jstreeContextMenuShowing = true;
				});
			}else{
				toggleSubmenuItem (items['ccp']['submenu']['Makehome'], items['ccp']['submenu']['MakehomeDisabled'], true);
				toggleSubmenuItem (items['ccp']['submenu']['Trash'], items['ccp']['submenu']['TrashDisabled'], true);
				toggleSubmenuItem (items['ccp']['submenu']['Checkin'], items['ccp']['submenu']['CheckinDisabled'], true);
		...
		function toggleSubmenuItem (menuItemEnabled, menuItemDisabled, hideEnabled, label)
		{
			if(hideEnabled){
				menuItemEnabled.show = true;
				menuItemDisabled.show = false;
				if(label){
					menuItemEnabled.label = label;
				}
			}else{
				menuItemEnabled.show = false;
				menuItemDisabled.show = true;
				if(label){
					menuItemDisabled.label = label;
				}				
			}
		}
	...
	*/
	var menuTypes = {
		...
		selectMenu: function(_selected){
			...
			menuTypeSelector.find('div#jsn-menutype-title-'+_selected).show();
		...
		initEvent: function(){
			...
			menuTypeSelector.unbind("mousedown").mousedown(function(e){
			...
			}).find('span.dropdown-arrow').unbind("click").click(function(e){
				e.stopPropagation();
				...
				if (menuTypeSelector.hasClass('jsn-menu-selector-disabled')){
					...
					menuTypesList.stop();
					menuTypesList
						.animate({ "height"  : menuHeight, "opacity" : 1 }, function () {
							menuTypeSelector.removeClass('jsn-menu-selector-disabled');
						});
				}
				else{
					menuTypesList.stop();
					menuTypesList.animate({ "height"  : 0, "opacity" : 0 }, function () {
	...
	_menuitems:{
		...
		*/	
		init: function(){
			...
			$._menuitems.headerContextMenu( $('.jsn-menutype-title') );
			...
			$('#menu-manager').unbind("click").click(function(){
				if ( !$(this).hasClass('enabled') ){
					...
					$(this).removeClass('enabled').addClass('disabled').removeAttr('class');
			...
			if ( $.jStorage.get('rawmode_showunpublished_menuitem') ){
				$('#menu-manager').removeClass('disabled').addClass('enabled').attr('title', JSNLang.translate('TITLE_HIDE_UNPUBLISHED_MENUITEMS'));
			}else{
				$('#menu-manager').removeClass('enabled').addClass('disabled').attr('title', JSNLang.translate('TITLE_SHOW_UNPUBLISHED_MENUITEMS'));
			}
					...
					if ( menuops.isNew()){
							...
							(
							 		...
							 		open  : function(){
									    ...
									    $(window).unbind("pressEnterSubmitForm").bind("pressEnterSubmitForm", function(){
											iframe.load(function(){
												...
												$('#jsn-menutype-title-'+menuid).html(newtitle);
												$('#dwopdownmenutype-'+menuid).children().html(newtitle);
												menuTypes.initEvent();
								...
								buttons: {									
									'Save': function(){
										...
										if (pop.submitForm('menu.apply', 'Save')){
											var newtitle = iframe.contents().find('input[name="jform[title]"]').val();
											iframe.load(function(){
												_this.removeClass('jsn-loading');
						...
						*/
						menuops.addItem('Rebuild').bind("click", function(){
							...
							$.post(baseUrl+'administrator/index.php?option=com_poweradmin&task=menuitem.rebuild', 
							{
								mid	:	menuid
							}).success(function(res){
								$.checkResponse(res);
						...
						var subPanelMore = menuops.addParentItem( JSNLang.translate('TITLE_SUBPANEL_MORE') );
							...
							*/
							subPanelMore.addItem( JSNLang.translate( 'TITLE_SUBMENUTITLE_DELETE' ) ).bind("click", function(){
								...
								if (answer) {
									$.post(baseUrl+'administrator/index.php?option=com_poweradmin&task=menuitem.deleteMenu', 
									{
										menuid:menuid
									}).success(function(res){
							...
							subPanelMore.addItem( JSNLang.translate( 'TITLE_SUBMENUTITLE_OPTIONS' ) ).bind("click", function(){
								...
								(
									baseUrl+'administrator/index.php?option=com_config&view=component&component=com_menus&path=&tmpl=component', 
									{
										...
										buttons:{											
											'Save & Close': function(){
												pop.submitForm('component.apply', 'Save & Close');
											},
											'Cancel': function(){
												$(this).dialog("close");
											}
						...
						try{
							if (  e.which === 1 && !e.ctrlKey){
								...
								$('body').unbind("click").bind("click", function(e){
									if ( !$(e.target).parents('div.jsn-menutype-title').length && !$(e.target).hasClass('jsn-menutype-title') ){
										menuops.hide({});
									}
								});
							}
		...
		*/
		getNewAdd: function(newId, menutype, title){
			$.post(baseUrl+'administrator/index.php?option=com_poweradmin&task=menuitem.getMenu', 
			{
				mid : newId
			}).success(function(res){
				$.checkResponse(res);
				if (res != 'error'){
					...
					$._menuitems.headerContextMenu($('#jsn-menutype-title-'+newId), false);
					//drop down list
					menuTypes.initEvent();
					setTimeout(function(){
		...
		*/	
		resetMenu: function(mid, _resetMenuCallbackFunc){
			...
			$.post(baseUrl+'administrator/index.php?option=com_poweradmin&task=menuitem.getMenuType', 
			{
				mid:mid
			}).success(function(res){
				...
				if (res != 'error'){
					try{
						...
						$._menuitems.headerContextMenu($('#jsn-menutype-title-'+mid), false);
		...
		*/
		selectmenutype: function(menutype, menutypeid, parentid){
			var pop = $.JSNUIWindow
			(
				baseUrl+'administrator/index.php?option=com_poweradmin&view=selectmenutypes&menutype='+menutype+'&menutypeid='+menutypeid+'&parentid='+parentid+'&tmpl=component&pwadvisual=1', 
				{
					modal : true,
					width : 750, 
					height: 550,
				...
				baseUrl+'administrator/index.php?option=com_poweradmin&task=selectmenutypes.setType&tmpl=component&params='+params+'&menutype='+menutype, 
				{
					...
					open  : function(){
						...
						iframe.load(function(){
							...
							iframe.contents().find('select#jform_parent_id').val(parentid);
							iframe.contents().find('input[id="jform_home0"]').click();
						});
					...
					buttons:{						
						'Save & Close': function(){
							if (pop.submitForm('item.save', 'Save & Close')){
								var _this  = $(this);
								var iframe = $(this).find('iframe');
								iframe.load(function(){
									$._menuitems.resetMenu(menutypeid);
