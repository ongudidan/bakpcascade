...
abstract class JSNInstallerScript
{
	...
	public function preflight($mode, $parent)
	{
		...
		// Make extension use shema mechanism
		if ($canInstall AND $canInstallExtension AND $canInstallSiteLanguage AND $canInstallAdminLanguage AND isset($extension->source))
		{
			...
			$q->from("#__extensions");
			$q->where("element='com_poweradmin'");
			$db->setQuery($q);
			$eid = $db->loadResult();
			if($eid){
				$q = $db->getQuery(true);
				...
				$db->setQuery($q);
				$versionid = $db->loadResult();
				if(!$versionid){
					$q = $db->getQuery(true);
					$q->insert('#__schemas');
					$q->set("extension_id=" . $eid.", version_id='1.0.0'" );
					$db->setQuery($q);
					if(!$db->execute()){
						return false;
					}
		...
		foreach ($this->dependencies AS $extension)
		{
			...
			if ($status = $db->loadObject())
			{
				...
				if (is_array($params = (isset($status->params) AND $status->params != '{}') ? (array) json_decode($status->params) : null))
				{
					...
					{
						if ($com != $this->name AND isset($params[$com]))
						{
							$dep[] = $com;
						}
					}
