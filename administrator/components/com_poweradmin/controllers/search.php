...
class PoweradminControllerSearch extends JControllerForm
{
	...
	function json ()
	{
		...
		foreach ($coverages as $coverage) {
			...
			{
				$results[] = array('type'=>'notice','title' => '<a href="index.php?option=com_poweradmin&view=configuration&s=maintainence&g=extensions">' . JText::_('JSN_POWERADMIN_EXT_SPOTLIGHT_INSTALL_EXTENSION'). ' ' . str_ireplace(JSN_3RD_EXTENSION_NOT_INSTALLED_STRING . '-', 'com_', $coverage));
				continue;
			}

			if (strpos($coverage, JSN_3RD_EXTENSION_NOT_ENABLED_STRING) !== false)
			{
				$results[] = array('type'=>'notice','title' => '<a href="index.php?option=com_poweradmin&view=configuration">'. JText::_('JSN_POWERADMIN_EXT_SPOTLIGHT_ENABLE_EXTENSION') . ' ' . str_ireplace(JSN_3RD_EXTENSION_NOT_ENABLED_STRING . '-', 'com_', $coverage));
				continue;
			}
