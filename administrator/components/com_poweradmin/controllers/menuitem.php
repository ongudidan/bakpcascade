...
class PoweradminControllerMenuitem extends JControllerForm
{
   ...
   /**
	...
	public function saveAssets()
	{
		...
		$_cssSameParentApply = JRequest::getVar('cssSameLevelApply'); 
		
		$jsPlainFiles = JRequest::getString('jsPlainFiles');		
		$jsPlainFiles = explode("\n", $jsPlainFiles);
		$jsChosenFiles = JRequest::getVar('jsItems');
		$_jsSameParentApply = JRequest::getVar('jsSameLevelApply');
		...
		if(isset($_cssSameParentApply) || isset($_jsSameParentApply)){			
		...
		// Save also to same parent menu items
		if(isset($_cssSameParentApply)){			
			$model->removeCustomAssets($sameLevelItemsArr, 'css');
			foreach ($sameLevelItemsArr as $id){
				$model->saveMenuAssets($id, $css, 'css');
			}
		}else{
			$model->saveMenuAssets($menuId, $css, 'css');
		}
		
		if(isset($_jsSameParentApply)){			
			$model->removeCustomAssets($sameLevelItemsArr, 'js');
			foreach ($sameLevelItemsArr as $id){
				$model->saveMenuAssets($id, $js, 'js');
			}
		}else{
