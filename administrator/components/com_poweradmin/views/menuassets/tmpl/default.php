...
if(count($this->jsFiles)){
	$initJsList = "<div id='jsn-tsn-js-item-list' class='jsn-items-list'>";
	foreach ($this->jsFiles as $key=>$value){
		...
		$initJsList .= "<div class='jsn-item ui-state-default'><label class='checkbox'><input type='checkbox' " . $checked . " name='jsItems[]' value='" . $key . "'>" . $key . "</input></label></div>";
	...
	<form action="index.php?option=com_poweradmin&task=menuitem.saveassets" name="adminForm" method="post">
		<input type="hidden" name="id" value="<?php echo JRequest::getInt('id'); ?>"/>
			...
			<div class="jsn-form-bar">
				<label  class="checkbox"><i class="icon-minus-sign"></i><input type="checkbox" class="assets-apply-all" name="cssSameLevelApply"/><?php echo JText::_('JSN_POWERADMIN_MENUASSETS_APPLY_TO_SAME_LEVEL_MENU')?></label>
			</div>
			 -->
			<hr>
			<div class="control-group">
			...
			<!-- 
			<div class="jsn-form-bar">
					<label  class="checkbox"><i class="icon-minus-sign"></i><input type="checkbox" class="assets-apply-all" name="jsSameLevelApply"/><?php echo JText::_('JSN_POWERADMIN_MENUASSETS_APPLY_TO_SAME_LEVEL_MENU')?></label>
				</div>	
				-->	
	</form>
</div>