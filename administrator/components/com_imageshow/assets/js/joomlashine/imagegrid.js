	...
	$.JSNISImageGrid = function( options ){
		...
		*/
		ImageGrid.initLayout      = function(){
			...
			if ( $.browser.msie ){
				ImageGrid.contents.showlistimages.parents('div.source-media-panel-container').css('margin-top', '-10px');
			}
			/**
...
//					//ImageGrid.saveShowlist();
		...
		ImageGrid.revertShowlistEmpty = function (){			
			...
			if(count == 0){				
				$('<div class="jsn-bglabel showlist-drag-drop-item-notice jsn-section-empty"><span class="jsn-icon64 icon-pointer"></span>'+JSNISLang.translate('SHOWLIST_NOTICE_DRAG_AND_DROP')+'</div>').appendTo(ImageGrid.showlistImages).show();
			}
		}
		/**
