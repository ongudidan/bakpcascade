...
/**
 * @version     $Id: submission.php 19014 2012-11-28 04:48:56Z thailv $
 * @package     JSNUniform
 * @subpackage  Models
...
class JSNUniformModelSubmission extends JModelAdmin
{
	...
	public function delete(&$pks = null, $formId = null)
	{
		...
		if (count($pks) && $formId)
		{
			$tblSubmitssion = '#__jsn_uniform_submissions_' . (int) $formId;
			foreach ($pks as $id)
			{
				$this->_db->setQuery('DELETE FROM ' . $tblSubmitssion . ' where data_id = ' . (int) $id);
				if (!$this->_db->execute())
				{
					return false;
				}
				$this->_db->setQuery('DELETE FROM #__jsn_uniform_data where data_id = ' . (int) $id);
				if (!$this->_db->execute())
				{
					return false;
				}
	...
	public function getDataSubmission()
	{
		if (!empty($this->_formId) && is_numeric($this->_formId) && !empty($this->_dataId) && is_numeric($this->_dataId))
		{
			...
			$this->_db->getQuery(true)
			->select('*')
			->from("#__jsn_uniform_submissions_{$this->_formId}")
			->where('data_id=' . intval($this->_dataId))
			);
