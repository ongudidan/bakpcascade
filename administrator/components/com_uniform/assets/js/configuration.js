/*------------------------------------------------------------------------
 ...
 # @version $Id: configuration.js 19094 2012-11-30 02:27:22Z thailv $
 -------------------------------------------------------------------------*/
define([
    'jquery',
    'uniform/uniform',
...
function ($, JSNUniform, JSNHelp, JSNUniformDialogEdition) {
    var JSNUniformConfigView = function (options) {
        ...
        this.init();
        $('#jsn-config-menu a#linkconfigs').bind('linkRequested', $.proxy(function () {
            this.options.currentAction = $('#jsnconfigform_action').val();
            this.init();
        }, this));
    }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success == true) {
                        spanMessage.addClass("label label-success").text(response.message);
                        $("#folder_upload_old").val($("#jsnconfig_folder_upload").val());

                    } else {
                        spanMessage.addClass("label label-important").text(response.message);
                    }
