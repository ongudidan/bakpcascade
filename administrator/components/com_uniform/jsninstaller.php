...
abstract class JSNInstallerScript
{
	...
	public function preflight($mode, $parent)
	{
		...
		$canInstall		= version_compare($joomlaVersion->RELEASE, (string) $this->manifest['version'], '>=') ? true : false;
...
<script type="text/javascript">
	...
	public function installExtension($extension)
	{
		...
		if (isset($status))
		{
			...
			$params = is_array($params)?array_merge($params, array_combine($status->custom_data, $status->custom_data)):array_combine($status->custom_data, $status->custom_data);
	...
	protected function updateExtension($extension)
	{
		...
		$condition = array('type' => $extension->type, 'element' => $extension->name);
	...
	protected function removeExtension($extension)
	{
		...
		if ($status = $db->loadObject())
		{
			...
			if (count($status->custom_data))
			{
				...
				// Backward compatible: keep data in this column also for another old product to recognize
				$params = is_array($params = (isset($status->params) AND $status->params != '{}')?(array) json_decode($status->params):null)?array_merge($params, array_combine($status->custom_data, $status->custom_data)):array_combine($status->custom_data, $status->custom_data);
	...
	protected function backupLanguage()
	{
		...
		$langUtil = (class_exists('JSNUtilsLanguage') AND method_exists('JSNUtilsLanguage', 'edited'))?'JSNUtilsLanguage':'JSNUtilsLanguageTmp';

		if ($langUtil != 'JSNUtilsLanguage')
		{
			file_exists($this->parentInstaller->getPath('source') . '/admin/libraries/joomlashine/utils/language.php')?require_once $this->parentInstaller->getPath('source') . '/admin/libraries/joomlashine/utils/language.php':($langUtil = null);
		}
		...
		if ($langUtil)
		{
			...
			foreach ($langs AS $lang)
			{
				...
				$isEdited = array('admin' => call_user_func(array($langUtil, 'edited'), $lang, false, "com_{$this->name}"), 'site' => call_user_func(array($langUtil, 'edited'), $lang, true, "com_{$this->name}"));
			...
			{
				$this->app->enqueueMessage('Cannot backup following user edited language file(s): <ul><li>' . implode('</li><li>', $backupFails) . '</li></ul>', 'warning');
	...
	protected function updateLanguage()
	{
		...
		if ($langUtil)
		{
			...
			foreach ($langs AS $lang)
			{
				...
				$isInstalled = array('admin' => call_user_func(array($langUtil, 'installed'), $lang, false, "com_{$this->name}"), 'site' => call_user_func(array($langUtil, 'installed'), $lang, true, "com_{$this->name}"));
			...
			if (isset($mergeFails))
			{
				$this->app->enqueueMessage('Cannot merge user edited translation back to following language file(s): <ul><li>' . implode('</li><li>', $mergeFails) . '</li></ul>', 'warning');
	...
	public static function jsnUniformRreFlight($componentInfo)
	{
		...
		if (!empty($componentInfo->version) && version_compare($componentInfo->version, '1.0.2', '<'))
		{
			jimport('joomla.filesystem.folder');
			jimport('joomla.filesystem.file');
			$folderUpload = JSNInstallerScript::getFolderUploadConfig();
			$folderUpload = !empty($folderUpload->value)?$folderUpload->value:'images/jsnuniform/upload/';
			$folderUrl = $folderUpload . '/jsnuniform_uploads/';
			$forms = JSNInstallerScript::getForms();
			if (!empty($forms))
			{
				foreach ($forms as $item)
				{
					...
					{
						$folderForm = JPath::clean(JPATH_ROOT . $folderUpload . '/' . $item->form_id);
						if (is_dir($folderForm))
						{
							if (JFolder::copy(JPath::clean($folderForm), JPath::clean(JPATH_ROOT . $folderUrl . '/' . $item->form_id), '', true, true))
							{
								JFolder::delete(JPath::clean($folderForm));
							}
						}
					}
				}
		...
		if (!empty($componentInfo->version) && version_compare($componentInfo->version, '1.0.6', '<'))
		{
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->update($db->quoteName("#__jsn_uniform_fields"));
			$query->set("field_type = " . $db->Quote("static-content"));
			...
			if ($data = $db->loadObjectList())
			{
				foreach ($data as $item)
				{
					...
					if (isset($item->page_content))
					{
						...
						if ($pageContent && (is_array($pageContent) || is_object($pageContent)))
						{
							foreach ($pageContent as $content)
							{
								...
								elseif ($content->type == 'static-paragraph')
								{
									$content->type = 'static-content';
								}
								elseif ($content->type == 'horizontal-ruler')
								{
									$content->type = 'static-content';
					...
					if (isset($item->page_template))
					{
						...
						if (isset($pageTemplate->dataField) && (is_array($pageTemplate->dataField) || is_object($pageTemplate->dataField)))
						{
							foreach ($pageTemplate->dataField as $template)
							{
								...
								elseif ($template->field_type == 'static-paragraph')
								{
									$template->field_type = 'static-content';
								}
								elseif ($template->field_type == 'horizontal-ruler')
								{
									$template->field_type = 'static-content';
					...
					$query->set("page_content = " . $db->Quote(json_encode($newContent)));
					$query->set("page_template = " . $db->Quote(json_encode($newTemplate)));
			...
			$db = JFactory::getDbo();
			$db->setQuery($db->getQuery(true)->select('field_type,form_id,field_id')->from("#__jsn_uniform_fields"));
			if ($dataFields = $db->loadObjectList())
			{
				$listTableSubmission = array();
				$listColumnSubmission = array();

				foreach ($dataFields as $item)
				{
					if (isset($item->field_type) && isset($item->form_id) && isset($item->field_id) && $item->field_type == "number")
					{

						$query = $db->getQuery(true);
						$tableName = $prefix . "jsn_uniform_submissions_" . $item->form_id;
						if (!in_array($tableName, $listTableSubmission))
						{
							...
							if ($db->loadResult())
							{
								$listTableSubmission[$tableName] = 1;
								$db->setQuery("SHOW COLUMNS FROM " . $db->quoteName($tableName));
								$listColumnSubmission[$tableName] = $db->loadObjectList();
								if (isset($listColumnSubmission[$tableName]))
								{
									foreach ($listColumnSubmission[$tableName] as $columnItems)
									{
										if (isset($columnItems->Field) && $columnItems->Field == "sb_" . $item->field_id)
										{
											$query = "ALTER TABLE " . $db->quoteName($tableName) . " CHANGE " . $db->quoteName("sb_" . $item->field_id) . " " . $db->quoteName("sb_" . $item->field_id) . " VARCHAR(255) NULL DEFAULT NULL";
											$db->setQuery($query);
											$db->query();
										}
									}
							...
							{
								$listTableSubmission[$tableName] = 0;
							}
						}
						elseif (isset($listTableSubmission[$tableName]) && $listTableSubmission[$tableName] == 1)
						{
							foreach ($listColumnSubmission[$tableName] as $columnItems)
							{
								if (isset($columnItems->Field) && $columnItems->Field == "sb_" . $item->field_id)
			...
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('TABLE_NAME')->from('INFORMATION_SCHEMA.TABLES')->where("TABLE_NAME LIKE " . $db->quote($prefix . "jsn_uniform_form_pages") . " AND TABLE_SCHEMA = " . $db->quote($dbName));
			$db->setQuery($query);
			if ($db->loadResult())
			{
				$query = $db->getQuery(true);
				$query = "ALTER TABLE " . $db->quoteName($prefix . "jsn_uniform_form_pages") . " CHANGE `page_content` `page_content` LONGTEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL , CHANGE `page_template` `page_template` LONGTEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL";
				$db->setQuery($query);
				$db->query();
			}
			...
			{
				if (isset($itemForm->Field) && $itemForm->Field == "form_btn_next_text" || $itemForm->Field == "form_btn_prev_text" || $itemForm->Field == "form_btn_submit_text")
				{
					$checkBtn = false;
				}
			}
			if ($checkBtn)
			{
				$query = "ALTER TABLE  " . $db->quoteName($prefix . "jsn_uniform_forms") . " ADD `form_btn_next_text` VARCHAR( 255 ) NOT NULL , ADD `form_btn_prev_text` VARCHAR( 255 ) NOT NULL , ADD `form_btn_submit_text` VARCHAR( 255 ) NOT NULL";
				$db->setQuery($query);
				$db->query();
			}
		}
		if (!empty($componentInfo->version) && version_compare($componentInfo->version, '1.1.1', '<'))
		{
			$db = JFactory::getDbo();
			$app = JFactory::getApplication();
			$prefix = $db->getPrefix();
			$dbName = $app->getCfg('db');
			$query = $db->getQuery(true);
			$query = "ALTER TABLE " . $db->quoteName($prefix . "jsn_uniform_fields") . " CHANGE `field_identifier` `field_identifier` VARCHAR( 255 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL";
			$db->setQuery($query);
			$db->query();

			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query = "ALTER TABLE " . $db->quoteName($prefix . "jsn_uniform_forms") . " CHANGE `form_style` `form_style` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL";
			$db->setQuery($query);
			$db->query();
		}
		...
		if (!empty($componentInfo->version) && version_compare($componentInfo->version, '1.3.0', '<'))
		{
			...
			if ($dataFormPage)
			{
				foreach ($dataFormPage as $page)
				{
					...
					$pageTempalte = json_decode($page->page_template);
					if ($pageContent)
					{
					...
					if (!empty($pageTempalte->dataField))
					{
						foreach ($pageTempalte->dataField as $j => $itemTemplate)
						{

							if (isset($itemTemplate->field_type) && $itemTemplate->field_type == "address")
							{
								$templateSettings = json_decode($itemTemplate->field_settings);
								$templateSettings->options->vstreetAddress = "1";
								$templateSettings->options->vstreetAddress2 = "1";
								$templateSettings->options->vcity = "1";
								$templateSettings->options->vcode = "1";
								$templateSettings->options->vstate = "1";
								$templateSettings->options->vcountry = "1";
								$itemTemplate->field_settings = json_encode($templateSettings);
								$pageTempalte->dataField[$j] = $itemTemplate;
							}
							if (isset($itemTemplate->field_type) && $itemTemplate->field_type == "name")
							{
								if (!empty($itemTemplate->options->format) && $itemTemplate->options->format == "Normal")
								{
									$templateSettings->options->vfirst = "1";
									$templateSettings->options->vlast = "1";
									$templateSettings->options->size = "jsn-input-small-fluid";
									$itemTemplate->field_settings = json_encode($templateSettings);
									$pageTempalte->dataField[$j] = $itemTemplate;
								...
								else
								{
									$templateSettings->options->vfirst = "1";
									$templateSettings->options->vlast = "1";
									$templateSettings->options->vmiddle = "1";
									$templateSettings->options->vtitle = "1";
									$templateSettings->options->size = "jsn-input-mini-fluid";
									$templateSettings->field_settings = json_encode($templateSettings);
									$pageTempalte->dataField[$j] = $itemTemplate;
								}
							}
						}
					...
					$page->page_template = json_encode($pageTempalte);
					$db = JFactory::getDbo();
					$query = $db->getQuery(true);
					$query->update($db->quoteName('#__jsn_uniform_form_pages'));
					$query->set("page_content = " . $db->Quote($page->page_content));
					$query->set("page_template = " . $db->Quote($page->page_template));
