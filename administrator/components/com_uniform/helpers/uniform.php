...
class JSNUniformHelper
{
	...
	{
		$db = JFactory::getDBO();
		$db->setQuery("SHOW COLUMNS FROM #__jsn_uniform_data");
	 ...
	 * @return  html code
	 */
	public static function getSelectForm($name, $id = null, $view = "", $value = null)
	{
		$enabledCSS = 'hide';
		...
		else if ($view == "menusubmissions")
		{
			...
			$html .= $listForm;
			$html .= "<span id =\"form-icon-warning\" class='{$enabledCSS}' title=\"" . JText::_('JSN_UNIFORM_FIELD_DES_FORM_WARNING') . "\"><span class=\"jsn-icon16 jsn-icon-warning-sign\"></span></span>";
			//$html .= "</div>" . JSNHtmlAsset::loadScript('uniform/menusubmissions', array('language' => JSNUtilsLanguage::getTranslated($arrayTranslated)), true);
			return $html;
		}
	 ...
	 * Load Page Form
	 *
	 * @param   Object  $items      Items Form
	 * @param   Array   $formPages  Form page
	...
	public static function generateHTMLPages($formId, $formName, $formType = "")
	{
		...
		if ($items)
		{
			...
			JSNHtmlAsset::addInlineStyle(JSNUniformHelper::generateStylePages($formStyleCustom, ".jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group", ".jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group.ui-state-highlight", ".jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group .control-label", ".jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group.error .help-block,.jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group.error .help-inline,.jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group.error .help-block span.label", ".jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group .label-important,.jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group .label-important .badge-important", ".jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group .controls input,.jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group .controls select,.jsn-master .jsn-bootstrap #form_{$formName} .jsn-form-content .control-group .controls textarea"));
			$dataSumbission = '';
			...
			if (!$formType)
			{
				...
				$html .= "<form name='form_{$formName}' id='form_{$formName}' action=\"" . JRoute::_('index.php?option=com_uniform&task=form.save&form_id=' . $items->form_id) . "\" method=\"post\" class=\"form-validate {$classForm} \" enctype=\"multipart/form-data\" >";
				$html .= "<div id=\"page-loading\" class=\"jsn-bgloading\"><i class=\"jsn-icon32 jsn-icon-loading\"></i></div>";
				$html .= "<div class=\"jsn-form-container {$formTheme}\">";
			}
				...
				if ($i + 1 == count($formPages))
				{
					...
					else if (!empty($items->form_captcha) && $items->form_captcha == 2)
					{
						...
						$html .= '<div class="control-group">
							...
							<input type="text" id="jsn-captcha" name="captcha" placeholder="' . JText::_("JSN_UNIFORM_CAPTCHA") . '">
							</div>
							</div>';
			...
			<div class="btn-toolbar">
			    <button class="btn prev hide" onclick="return false;">' . JText::_($btnPrev) . '</button>
			    <button class="btn next btn-primary hide" onclick="return false;">' . JText::_($btnNext) . '</button>
