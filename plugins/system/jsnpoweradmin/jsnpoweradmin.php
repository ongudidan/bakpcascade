	...
	*/
	public function onBeforeRender()
	{
		...
		if(!$this->_application->isAdmin() && $this->_params->get('custom_assets_enhance', true)){
			...
			if(count($menuCss)){
				foreach ($menuCss as $key=>$value){
					if($value->loaded === "true"){
						$this->_document->addStyleSheet($key);
					}
				}
			}
			if(count($menuJs)){
				foreach ($menuJs as $key=>$value){
					if($value->loaded === "true"){
						$this->_document->addScript($key);
					}
				}
			}
		}
...
// 		if ($this->_application->isAdmin() AND $this->_application->input->getCmd('option') == $this->_ext)
// 		{
// 			// Generate path to component initialization file
// 			$path = JPATH_ADMINISTRATOR . "/components/com_poweradmin/bootstrap.php";

// 			// Require the initialization file file for execution
// 			require_once $path;
// 		}
