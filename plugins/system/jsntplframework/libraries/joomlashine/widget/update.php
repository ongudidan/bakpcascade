...
class JSNTplWidgetUpdate extends JSNTplWidgetBase
{
	...
	public function installPackageAction ()
	{
		...
		$source = $joomlaConfig->get('tmp_path') . '/' . $this->template['name'] . '_backup.zip';
		$target = $templatePath . '/backups/' . date('y-m-d') . '.zip';
		...
		$manifest = JSNTplHelper::getManifest($this->template['name']);
