...
class JSNTplTemplateHelper
{
	...
	private function _prepare ($loadTemplateCSS, $loadTemplateJS)
	{
		...
		$templateParams = $this->_document->params;
		$templateName   = $this->_document->template;

		if (empty($templateParams) || empty($templateName))
		{
	...
	private function _prepareHead ()
	{
		...
		if ($this->_loadTemplateJS == true)
		{
			...
			// $this->_document->addScript($this->_document->rootUrl . '/media/system/js/mootools-more.js');
			...
			$this->_document->addScriptDeclaration('
				JSNTemplate.initTemplate({
					...
					enableMobile			: ' . (isset($this->_document->mobileView) ? 'true' : 'false') . ',
