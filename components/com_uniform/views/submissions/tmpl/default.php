...
$listPositionField = $this->escape($this->_state->get('filter.position_field' . $this->_Itemid));
$listViewField = $this->escape($this->_state->get('filter.list_view_field' . $this->_Itemid));
$dateSubmission = $this->escape($this->_state->get('filter.date_submission' . $this->_Itemid));
$formId = $this->_formId;
	...
	<form action="<?php echo JRoute::_('index.php?option=com_uniform&view=submissions'); ?>" class="form-inline" method="post" id="adminForm" name="adminForm">
	    <fieldset class="jsn-fieldset-filter">
			...
			<div class="pull-left jsn-fieldset-search">
				...
				{
					?>
				    <input type="text" class="input-medium" name="filter_search<?php echo $this->_Itemid;?>" id="filter_search" value="<?php echo $this->escape($this->_state->get('filter.search' . $this->_Itemid)); ?>" title="<?php echo JText::_('JSN_UNIFORM_FORM_SEARCH_IN_TITLE'); ?>" />
					<?php
				}
				if ($showDateFilter == "1")
				{
					?>
				    <input type="text" class="input-medium" placeholder="- <?php echo JText::_('JSN_UNIFORM_PLACEHOLDER_SELECT_DATE'); ?> -" name="filter_date_submission<?php echo $this->_Itemid;?>" id="filter_date_submission" value="<?php echo $this->escape($this->_state->get('filter.date_submission' . $this->_Itemid)); ?>" title="<?php echo JText::_('JSN_UNIFORM_FORM_SEARCH_IN_DATE_SUBMISSION'); ?>" />
					<?php
				}
				?>
			...
			<div id="submission-fields-list" class="jsn-master jsn-bootstrap">
				...
				<div class="popover-content">
					...
					for ($i = 0; $i < count($fieldIdentifier); $i++)
					{
						...
						$items .= '<div class="' . $styleClass[$i] . ' jsn-item ui-state-default"><label class="checkbox"><input ' . $checked . ' type="checkbox" title="' . $fieldTitle[$i] . '" name="field[]" value="' . $fieldIdentifier[$i] . '">' . JText::_($fieldTitle[$i]) . '</label></div>';
					}
					?>
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						if (is_array($arrayField))
						{
							...
							foreach ($arrayField as $j => $field)
							{
								...
								{
									$contentField = JSNUniformHelper::getDataField($fieldType[$field], $item, $field, $formId, true);
									$contentField = $contentField?$contentField:"<span>N/A</span>";
									if ($j < 1)
									{
										$contentField = '<td><a href="' . JRoute::_('index.php?option=com_uniform&view=submission&data_id=' . (int) $item->data_id) . '">' . $contentField . '</a> </td>';
									}
									elseif ($field == 'data_created_by' && !$item->$field)
									{
		...
		<input type="hidden" name="list_view_field<?php echo $this->_Itemid;?>" id="list_view_field" value="<?php echo $listViewField; ?>" />
		<input type="hidden" name="filter_position_field<?php echo $this->_Itemid;?>" id="filter_position_field" value="<?php echo $listPositionField; ?>" />
